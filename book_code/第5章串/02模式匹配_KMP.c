#include <string.h>	/*strlen函数的使用*/
#include <stdio.h>    
#include <stdlib.h>	/*malloc free函数的使用*/   
...
{
	int i,*p;
	String s1,s2;	/* 相当于char s1[MAXSIZE+1]; 和 char s2[MAXSIZE+1]; */
	
	StrAssign(s1,"abcdex");	/* C 语言会自动将这个字符串常量的首地址（也就是字符 'a' 的地址）传递给对应的参数chars */
	printf("子串为: ");
	StrPrint(s1);
	i=StrLength(s1);
